...
app.layout = html.Div(
    children=[
        ...
        # # output provider (confirm button)
        # html.Div([
...
@app.callback(
    ...
    Input('flw', 'value'),
    Input('flwer', 'value'),
    # Input("outputprovider", "submit_n_clicks")
    Input("btn", "n_clicks")
    ],
    prevent_initial_call=True,)
# callback!(app, Output("output-provider", "children"),
#               Input("danger-danger-provider", "submit_n_clicks")) d
#               submit.n.clicks
...
#     end
# end
# def output(n_clicks, profile,nlu,dl,url,po,flw,flwer):
#     if n_clicks:
#         return ""
#     else:
...
def cond(btn,profile,nlu,dl,url,po,flw,flwer):
